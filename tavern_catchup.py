...
def main():
    ...
    # 邊界：判別器必須分得出**管線**與**重導向到檔案** —— 兩者 `isatty()` 都是 False，
    #      但只有前者會截斷。
    # 🩸 首版用 `isatty()`，於是 `> file`（本工具自己推薦的安全寫法）也被警告，
    #   而那行字的三個宣稱在那個情境下**全錯**：沒被截斷、cursor 有推進、這次讀到的算已讀
    #   （@basecamp 2026-08-15 22:53 當場實測，回讀 `_inbox_cursor` 確認）。
    #   ⇒ 那是今天第③隻的近親：**用宣稱的語氣講一件它沒查的事**（它沒看 cursor，是從 isatty 推的）。
    #   ⇒ 而假陽性的代價是方向性的：**它會教人忽略那行字，然後在真正該救的那次照樣被忽略。**
    # ⇒ 改用 `S_ISFIFO`（實測 Windows 可分：`| cat` → FIFO=True／`> file` → REG=True）。
    # ⚠ 仍是**事前推測**（管線 ≠ 一定截斷，`| cat` 就不會），所以措辭只講風險，不宣稱結果。
    #   真正的事後事實（cursor 到底有沒有推進）要落磁碟、由下一次叮報 —— 未做，見下。
    # TODO(未做，別當它已解)：EPIPE 時把「本次未推進 cursor」寫進磁碟，下一次叮開頭讀出來報。
    #   理由：管線斷掉時 stdout 已經死了，**唯一還活著的通道是磁碟**。
    try:
        import stat as _stat
        if _stat.S_ISFIFO(os.fstat(sys.stdout.fileno()).st_mode):
            print("⚠ 輸出接了管線。若下游會提早關閉（`| head` / `| tail`）：**輸出會被截斷，"
                  "而且 cursor 不會推進**（那次讀到的不算已讀）。想少讀請用 `--limit N`。")
    except Exception:
        pass
